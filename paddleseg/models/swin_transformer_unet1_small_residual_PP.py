...
class PatchEmbed(nn.Layer):
    ...
    def __init__(self, patch_size=4, in_chans=64, embed_dim=128, norm_layer=None):
        super().__init__()
...
class SwinTransformerUnet1_small_residual_PP(nn.Layer):
    """
    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                dim=int(embed_dim * 2**i_layer) if i_layer!=2 else 256,
    ...
    def train(self):
        ...
        super(SwinTransformerUnet1_small_residual_PP, self).train()
        self._freeze_stages()
...
class Encoder(nn.Layer):
    def __init__(self, in_channels=4):
        ...
        self.double_conv = nn.Sequential(
            layers.ConvBNReLU(in_channels, 32, 3), layers.ConvBN(32, 32, 3))
        down_channels = [[32, 64], [64, 128]]
        self.down_sample_list = nn.LayerList([
            self.down_sampling(channel[0], channel[1])
            for channel in down_channels
        ])
        self.conv_1_1_list = nn.LayerList([
            layers.ConvBN(channel[0], channel[1], 1)
            for channel in down_channels
        ])

        self.downsample = nn.MaxPool2D(kernel_size=2, stride=2)
        self.add = layers.Add()
        self.ac = layers.Activation(act="relu")
        self.conv1_1 = layers.ConvBN(in_channels, 32, 1)

    def down_sampling(self, in_channels, out_channels):
        modules = []
        # modules.append(nn.MaxPool2D(kernel_size=2, stride=2))
        modules.append(layers.ConvBNReLU(in_channels, out_channels, 3))
        modules.append(layers.ConvBN(out_channels, out_channels, 3))
        return nn.Sequential(*modules)


    def forward(self, x):
        short_cuts = []
        x_res = self.conv1_1(x)
        x = self.double_conv(x)
        x = self.add(x,x_res)
        x = self.ac(x)
        # for [down_sample,conv_1_1] in [self.down_sample_list,self.conv_1_1_list]:
        #     short_cuts.append(x)
        #     x = self.downsample(x)
        #     x_res = conv_1_1(x)
        #     x = down_sample(x)
        ...
        for i in range(2):
            short_cuts.append(x)
            x = self.downsample(x)
            x_res = self.conv_1_1_list[i](x)
            x = self.down_sample_list[i](x)
            x = self.add(x, x_res)
            x = self.ac(x)
        return x, short_cuts
...
class Decoder(nn.Layer):
    def __init__(self, align_corners, use_deconv=False):
        ...
        up_channels = [[256, 128], [128, 64], [64, 32], [32, 32]]
...
class UpSampling(nn.Layer):
    def __init__(self,
        ...
        self.double_conv = nn.Sequential(
            ...
            layers.ConvBN(out_channels, out_channels, 3))
        self.conv_1_1 = layers.ConvBN(in_channels, out_channels, 1)
        self.add = layers.Add()
        self.ac = layers.Activation(act="relu")
